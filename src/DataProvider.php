    ...
    private function load(): void
    {
        ...
        foreach ($specialRulesRecords as $record) {
            $specialRule = new SpecialRule(
                SpecialRuleCategory::from($record['Category']),
                $record['Name'],
        ...
        foreach ($teamRecords as $record) {
            $teamSpecialRules = [];
            preg_match('/(?P<type>all_of|one_of)=\[(?P<rules>.+)?]/', $record['Special Rules'], $matches);
            foreach($matches as $match) {
                $rules = array_filter(array_map('trim', explode(",", $matches['rules'])));
        ...
        foreach ($starPlayerPositionRecords as $record) {
            ...
            $record['Plays For Rule'] = str_replace(
                'All',
                'Badlands Brawl, Elven Kingdoms League, Halfling Thimble Cup, Lustrian Superleague, Old World Classic, Sylvanian Spotlight, Underworld Challenge, Worlds Edge Superleague',
                $record['Plays For Rule']
            );
